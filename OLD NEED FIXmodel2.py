def main2():
  ...
  api_url = 'https://api.hockeychallengehelper.com/api/picks'
  response = requests.get(api_url)

  # Parse the JSON data from the API response
  data = json.loads(response.text)

  # Iterate through the data to extract player names
  player_names = []

  for player_list in data['playerLists']:
  ...
  total_length = len(player_names)

  # Calculate the sizes of the three base lists
  size1 = total_length // 3
  size2 = (total_length // 3) + (total_length % 3 >= 1)
  size3 = (total_length // 3) + (total_length % 3 >= 2)

  # Divide the list into three roughly equal parts
  base1 = player_names[:size1]
  base2 = player_names[size1:size1 + size2]
  base3 = player_names[size1 + size2:]

  # Print the three lists to check
  print("Base 1:", base1)
  print("Base 2:", base2)
  print("Base 3:", base3)
  print(len(base3))
  ...
  for player_list in lists:
    ...
    for i in player_list:
      ...
      for split in json_data['stats'][0]['splits']:
        ...
        opp_ga_per_gp = opp_data['teams'][0]['teamStats'][0]['splits'][0][
      ...
      shooting_pct = float(
        json_data["stats"][0]["splits"][0]["stat"]["shotPct"])
      ...
      goalsAgainstPerGame = data["stats"][0]["splits"][0]["stat"][
        "goalsAgainstPerGame"]
      ...
      with open("mcdavid_2019_data.csv") as csvfile:
        ...
        if total < 2:
          ...
          print(
            f"The predicted odds of {str(full_name)} scoring in a today's game is {0:.2%}"
          )
      ...
      while average <= 30:
        ...
        while retry_count < max_retry:
          try:
            random_state = random.randint(
              1, 100)  # Generate a random random_state value
            ...
            print(
              f"The predicted odds of {str(full_name)} scoring in a today's game is {odds:.2%}"
            )
        ...
        if retry_count == max_retry:
          print(
            "Maximum retries reached. Could not generate suitable random_state value."
          )
